...
//           <Link href={window.open(`/crypto-ico-details/${post.slug}`, '_blank')}>{post.slug}</Link>
//         </td>
//         <td>{post.alias}</td>
...
//                 `/api/admin/crypto_coins?page=${page}&limit=${postsPerPage}&search=${encodeURIComponent(search)}`
//             );
...
//     // Fetch posts when page or search query changes
//     useEffect(() => {
...
//         e.preventDefault();
//         // The actual search is triggered by the useEffect that watches debouncedSearchQuery
//     };
...
const CryptoCoins = () => {
    ...
    const fetchPosts = useCallback(async (page = 1, filters = {}) => {
        try {
            ...
            const response = await axios.get(`/api/admin/crypto_coins?${params.toString()}`);
