...
const RegisterNewCoins = () => {
    ...
    const fetchProjects = async (page = 1, limit = itemsPerPage, search = searchQuery) => {
        setLoading(true);
        try {
            const response = await axios.get('/api/admin/register-new-coins', {
                params: {
                    page, 
                    limit,
                    search
    ...
    const handleAddProject = () => {
        router.push('/admin/dashboard/crypto/register-new-coins/add');
    };

    // Handle project viewing
    const handleViewProject = (id) => {
        router.push(`/admin/dashboard/crypto/register-new-coins/view/${id}`);
    };

    // Handle project editing
    const handleEditProject = (id) => {
        router.push(`/admin/dashboard/crypto/register-new-coins/edit/${id}`);
    };
    ...
    const handleDeleteProject = async () => {
        ...
        try {
            const response = await axios.delete(`/api/admin/register-new-coins/${projectToDelete.id}`);
    ...
    return (
        <Fragment>
            <Seo title="Register New Coins" />
            <PageHeader title='Register New Coins' item='Register New Coins' active_item='All Register New Coins' />
            <ToastContainer />
                    ...
                    <Card className="custom-card">
                        <Card.Header className="border-bottom-0 pb-0">
                            <div className="d-flex justify-content-between align-items-center w-100">
                                <label className="main-content-label my-auto mb-2">Register New Coins</label>
                                    ...
                                    <Table className="table-bordered bg-light">
                                        <thead>
                                            <tr>
                                                ...
                                                <th className="bg-primary">Logo</th>
                                                <th className="bg-primary">Funding Stage</th>
                                                <th className="bg-primary">Coin Name</th>
                                                <th className="bg-primary">Symbol</th>
                                                <th className="bg-primary">Total Coin</th>
                                        ...
                                        <tbody className="mb-3 fs-6">
                                            {projects.length > 0 ? (
                                                projects.map((project) => (
                                                    <tr key={project.id}>
                                                        <td>
                                                           <Image src={project.image} alt="coin logo" height={30} width={30} /></td>
                                                        <td>{handleType(project.fund_stage)}</td>
                                                        <td>{project.coin_name}</td>
                                                        <td>{project.ticker}</td>
                                                        <td>{project.total_coin}</td>
...
export default RegisterNewCoins;