...
export async function GET() {
  try {
    ...
    const formattedPosts = guestPosts.map(post => ({
      ...
      date: post.created_at ? formatISODate(post.created_at) : formatISODate(new Date()),
      likes: post.likes_counts || 0,
      slug: post.alias || "abc",
      seo: post.is_seo_check || 0,
      ...
      lan: post.lang || "en",
      is_new_blog: post.new_article || 0,
      publishedAt: post.publishedAt ? formatDateWithTime(post.publishedAt) : formatDateWithTime(new Date()),
      uploadedAt: post.created_at ? formatISODate(post.created_at) : formatISODate(new Date()),
