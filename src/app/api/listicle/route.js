...
//       query: "SELECT * FROM `crypto_news` WHERE is_gabbar = 3 ORDER BY created_at DESC"
//     });
...
export async function GET() {
  try {
    ...
    console.error('Error fetching Listicle posts:', error);
    return NextResponse.json(
      {
        success: false,
        message: 'Failed to fetch Listicle posts'
      }, 
      { status: 500 }
    );
  } finally {
